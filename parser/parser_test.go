...
func TestValues_Parse(t *testing.T) {
	...
	tests := []struct {
		...
		// Arrecadacao/convenio (utility, city hall, telecom, government
		// agency bills) — same inputs as TestValues_GetBoletoType below,
		// now also asserting the decoded amount/due-date fields, which
		// Parse() previously never populated for this boleto family at
		// all. Most of these don't encode a due date in the free field
		// (optional per FEBRABAN spec) so DueDate is expected to stay
		// zero-value; amount is always present and mandatory.
		{"826700000035 645607980002 010002351038 822024116714",
			&utils.Boleto{
				GeneralCheckDigit: 7,
				Amount:            364.56,
				CodeType:          "DIGITABLE_LINE",
				Type:              utils.Sanitation,
			},
		},
		...
		{"856500000026 056505152027 411292024030 335182000000",
			&utils.Boleto{
				GeneralCheckDigit: 5,
				Amount:            205.65,
				DueDate:           time.Date(2024, 11, 29, 0, 0, 0, 0, loc),
				CodeType:          "DIGITABLE_LINE",
				Type:              utils.GovernmentAgencies,
			},
		},
		{"846800000008 550000791008 011193989719 924101544345",
			&utils.Boleto{
				...
				Amount:            55.00,
				CodeType:          "DIGITABLE_LINE",
				Type:              utils.Telecommunications,
			},
		},
		// Real Claro/NET internet bill (hand-verified against the actual
		// document: "Valor Total 149,90", bill dated 25/07/2026).
		{"84660000001-8 49900162202-0 60725379000-2 00365991394-5",
			&utils.Boleto{
				GeneralCheckDigit: 6,
				Amount:            149.90,
				DueDate:           time.Date(2026, 7, 25, 0, 0, 0, 0, loc),
				CodeType:          "DIGITABLE_LINE",
				Type:              utils.Telecommunications,
			},
		},
